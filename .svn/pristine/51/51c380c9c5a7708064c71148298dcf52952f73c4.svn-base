...
class Powerup(pygame.sprite.Sprite):
    ...
    def enableBulletBuff(self, enemies, powerupAmount):
        for enemy in enemies:
            enemy.damageFactor += powerupAmount
        time.sleep(5)
        for enemy in enemies:
            enemy.damageFactor -= powerupAmount

    def doPowerup(self, player, enemies):
        amount = 0
        ...
        #TODO: FINISH IMPLEMENTING THIS PART
            ...
            if powerupAmount is not '':
                t = threading.Thread(target=self.enableBulletBuff, args=(enemies, int(powerupAmount)))
                t.start()
